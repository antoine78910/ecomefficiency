(function() {
    ...
    const PIPIADS_CREDENTIALS = {
        email: 'efficiencyecom@gmail.com',
        password: 'yK9cCyuLTL8NZ2x'
    };
    ...
    async function performAutoLogin() {
        try {
            ...
            // Chercher l'email avec plusieurs sélecteurs robustes
            const findEmail = () => (
                document.querySelector('input[placeholder="Veuillez saisir votre adresse e-mail"]') ||
                document.querySelector('input[placeholder*="adresse e-mail" i]') ||
            ...
            const findPassword = () => (
                document.querySelector('input[placeholder="Veuillez saisir votre mot de passe"]') ||
                document.querySelector('input[placeholder*="mot de passe" i]') ||
                document.querySelector('input[placeholder*="password" i]') ||
                document.querySelector('input[type="password"]') ||
                document.querySelector('input[name*="password" i]')
            );

            const emailInput = await new Promise((resolve, reject) => {
                const el = findEmail();
                ...
                setTimeout(() => { obs.disconnect(); reject(new Error('Email field not found')); }, 15000);
            });

            const passwordInput = await new Promise((resolve, reject) => {
                const el = findPassword();
                if (el) return resolve(el);
                const obs = new MutationObserver(() => {
                    const e = findPassword();
                    if (e) { obs.disconnect(); resolve(e); }
                });
                obs.observe(document.body, { childList: true, subtree: true });
                setTimeout(() => { obs.disconnect(); reject(new Error('Password field not found')); }, 15000);
            });
