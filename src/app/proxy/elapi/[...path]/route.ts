...
// Primary API region observed in logs
const API_BASES = [
...
async function forward(method: string, req: NextRequest, ctx: Ctx) {
  ...
  // Keep the first base; could be improved to detect region dynamically
  const upstreamUrl = new URL(p + (url.search || ''), API_BASES[0])
