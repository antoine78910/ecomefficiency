  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // Realtime plan via Stripe verify endpoint
        try {
          ...
          if (j?.ok && j?.active && (p === 'starter' || p === 'pro' || p === 'growth')) setPlan((p==='growth'?'pro':p) as any);
            ...
            const metaPlan = (meta.plan as string)?.toLowerCase();
            if (metaPlan === 'starter' || metaPlan === 'growth' || metaPlan==='pro') setPlan((metaPlan==='growth'?'pro':metaPlan) as any);
            else setPlan('free');
          }
        } catch {
    ...
    <div className="sticky top-0 z-50 w-full h-16 border-b border-[#5c3dfa]/20 bg-black/50 backdrop-blur flex items-center justify-between pl-5 pr-3 gap-4">
      <div className="flex items-center gap-2">
        <div className="rounded-xl overflow-hidden">
          <Image src="/lovable-uploads/4df3c2ff-c349-471e-bd38-1b3a1d95b2c2.png" alt="Ecom Efficiency Logo" width={140} height={56} className="h-14 w-auto rounded-xl mix-blend-screen" />
        </div>
      </div>
      {email ? (
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <button className="mt-2 focus:outline-none">
              <span className="flex items-center gap-2 rounded-full border border-white/10 bg-white/5 px-3 py-2">
                ...
                <span className="text-sm text-[#cfd3d8]">{firstName || email}</span>
              </span>
            </button>
          </DropdownMenuTrigger>
          <DropdownMenuContent align="end" className="w-64 bg-[#202031]/95 backdrop-blur-xl border-[#5c3dfa]/40 text-white">
            <div className="px-3 py-2 text-sm border-b border-[#5c3dfa]/20">
              Signed in as<br />
              <span className="font-medium">{firstName || email}</span>
            </div>
