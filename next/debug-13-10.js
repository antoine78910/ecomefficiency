// Script de debug pour voir exactement qui a rejoint le 13-10
// Usage: node debug-13-10.js
...
async function debug13October() {
  console.log('🔍 Debug du 13 octobre 2025...\n')
  ...
  try {
    ...
    // Filtrer ceux du 13-10
    const oct13Members = members.filter(member => {
      const joinedAt = new Date(member.joined_at)
      const joinDate = joinedAt.toISOString().slice(0, 10)
      return joinDate === '2025-10-13'
    })
    
    console.log(`\n📅 Membres du 13 octobre 2025: ${oct13Members.length}`)
    
    oct13Members.forEach((member, index) => {
