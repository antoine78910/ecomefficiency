...
export async function POST(req: NextRequest) {
  ...
  if (!process.env.STRIPE_SECRET_KEY || secrets.length === 0) {
    console.error('[webhook]', requestId, 'ÔØî Stripe not configured');
    ...
    switch (event.type) {
      case 'checkout.session.completed': {
        ...
        if (paymentStatus === 'paid' && status === 'complete') {
          ...
          if (userEmail) {
             console.log('[webhook][checkout.session.completed]', requestId, '­ƒÜÇ Tracking payment_succeeded in Brevo for:', userEmail);
        ...
        if (paymentStatus === 'paid' && status === 'complete') {
          ...
          if (!targetUserId && userEmail && partnerSlug && supabaseAdmin) {
            try {
          ...
          if (targetUserId && process.env.SUPABASE_SERVICE_ROLE_KEY && process.env.NEXT_PUBLIC_SUPABASE_URL) {
            await fetch(`${process.env.NEXT_PUBLIC_SUPABASE_URL}/auth/v1/admin/users/${targetUserId}`, {
              method: 'PUT',
              headers: {
      ...
      case 'customer.subscription.updated': {
        ...
        // D├ëTECTION : L'utilisateur vient de cliquer sur "Annuler" (cancel_at_period_end passe de false ├á true)
        if (subscription.cancel_at_period_end && previousAttributes && 'cancel_at_period_end' in previousAttributes && previousAttributes.cancel_at_period_end === false) {
           console.log('[webhook]', requestId, '­ƒôë User scheduled cancellation (Churn Risk)');
           ...
           try {
             ...
             if (userEmail) {
                ...
                console.log('[webhook]', requestId, 'Ô£à Tracked subscription_cancel_initiated for:', userEmail);
             }
           } catch (e) {
      ...
      case 'invoice.payment_succeeded': {
        ...
        if (!subscriptionId) {
          console.log('[webhook][invoice.payment_succeeded]', requestId, 'ÔÜá´©Å No subscriptionId in invoice, attempting to retrieve via customer or metadata');
        }
        ...
        if (subscriptionId) {
          try {
            ...
            if (targetUserId && process.env.SUPABASE_SERVICE_ROLE_KEY && process.env.NEXT_PUBLIC_SUPABASE_URL) {
              const updateRes = await fetch(`${process.env.NEXT_PUBLIC_SUPABASE_URL}/auth/v1/admin/users/${targetUserId}`, {
                method: 'PUT',
                headers: {
                  'Content-Type': 'application/json',
                  'Authorization': `Bearer ${process.env.SUPABASE_SERVICE_ROLE_KEY}`
              ...
              const userRes = await fetch(`${process.env.NEXT_PUBLIC_SUPABASE_URL}/auth/v1/admin/users/${targetUserId}`, {
                headers: {
                  'Authorization': `Bearer ${process.env.SUPABASE_SERVICE_ROLE_KEY}`,
        ...
        if (userEmail) {
            console.log('[webhook][invoice.payment_succeeded]', requestId, '­ƒÜÇ Calling trackBrevoEvent for:', userEmail);
            await trackBrevoEvent({
              email: userEmail,
              eventName: 'payment_succeeded',
              eventProps: {
            ...
            console.log('[webhook][invoice.payment_succeeded]', requestId, 'Ô£à Tracked payment_succeeded in Brevo for:', userEmail);
            ...
            await fetch(`${process.env.NEXT_PUBLIC_SUPABASE_URL?.replace('/auth/v1', '') || 'https://app.ecomefficiency.com'}/api/send-welcome-email`, {
             ...
             console.log('[webhook][invoice.payment_succeeded]', requestId, 'ÔØî Skipped Brevo tracking - No Email Found');
        }
      ...
      case 'customer.subscription.deleted': {
        ...
        try {
           ...
           if (userEmail) {
              ...
              console.log('[webhook]', requestId, 'Ô£à Tracked subscription_cancelled for:', userEmail);
           } else {
              console.log('[webhook]', requestId, 'ÔÜá´©Å Could not find email for cancelled subscription:', subscription.id);
           }
        } catch {}
        break;
      }
