...
function escapeEmailForStripeSearch(email: string) {
  return String(email || "")
    .trim()
    .replace(/\\/g, "\\\\")
    .replace(/'/g, "\\'");
 ...
 */
function stripeCustomerSearchEmailVariants(raw: string): string[] {
...
/** All customers matching email (paginated) — same email can have multiple Customer records after cancel/resubscribe */
...
async function searchCustomersByEmailAllPagesMerged(
  stripe: Stripe,
  email: string,
  requestOptions?: Stripe.RequestOptions
): Promise<Stripe.Customer[]> {
...
async function findValidSubscriptionForCustomer(
  stripe: Stripe,
  customerId: string,
  requestOptions?: Stripe.RequestOptions
...
async function findBestCustomerWithActiveSubscription(
  stripe: Stripe,
