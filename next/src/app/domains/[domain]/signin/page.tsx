...
import DomainSignInClient from "./DomainSignInClient";

export const runtime = "nodejs";
export const dynamic = "force-dynamic";

export default async function DomainSignInPage({ params }: { params: Promise<{ domain: string }> }) {
  ...
  return (
    <div style={{ ["--wl-main" as any]: String(colors?.main || ""), ["--wl-accent" as any]: String(colors?.accent || "") } as any}>
      <DomainSignInClient title={title} subtitle={subtitle} logoUrl={logoUrl} colors={colors} />
    </div>
  );
}
