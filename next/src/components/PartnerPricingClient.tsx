...
export default function PartnerPricingClient({ config }: { config: PartnerPricingConfig }) {
  ...
  const [status, setStatus] = React.useState<{ connected: boolean; bankLast4?: string | null }>({ connected: false });
  ...
  const yearly =
    explicitYearly > 0
      ? explicitYearly
      : Math.round(monthly * 12 * (1 - Math.min(Math.max(annualDiscount, 0), 90) / 100) * 100) / 100;
      ...
      <div className="flex items-center justify-between gap-3 mb-3">
        ...
        {hasYearly ? (
          <div className="text-xs text-gray-400">
            ...
            {annualDiscount > 0 && !(explicitYearly > 0) ? <span className="text-green-300"> • {annualDiscount}% off</span> : null}
          </div>
        ) : null}
      </div>
